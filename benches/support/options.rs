...
impl Options {
    ...
    pub fn parse_from(args: impl IntoIterator<Item = String>) -> Result<Option<Self>> {
        ...
        while let Some(arg) = args.next() {
            ...
            if arg == "--help" {
                eprintln!(
                    ...
                    --filter-every N (search only; select document IDs divisible by N)\n\
        ...
        if filter_explicit && (o.filter_every == 0 || o.scenario != "search") {
            return Err("filter-every requires a positive N and search".into());
        }
